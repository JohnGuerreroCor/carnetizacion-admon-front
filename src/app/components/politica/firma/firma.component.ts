...
export class FirmaComponent implements OnInit {
  ...
  displayedColumns: string[] = ['index', 'nombre', 'inicio', 'fin', 'opciones'];
  ...
  datosAdministrador() {
    this.personaService
      ...
      .subscribe((data) => {
        this.persona = data;
        this.administradorServices
          .getAdministrativo(data[0].identificacion)
          .subscribe((data) => {
            this.admin = data;
          });
      });
  ...
  buscarRectorActual() {
    this.firmaService.obtenerRectorActual().subscribe((data) => {
      ...
      this.formRector
        .get('nombre')!
        .setValue(data[0].persona.nombre + ' ' + data[0].persona.apellido);
  ...
  buscarPoliticaEstamento() {
    this.politicaEstamentoService
      .obtenerPoliticaEstamento()
      .subscribe((data) => {
  ...
  parametrosFirmaRegistrar(): void {
    ...
    firma.nombreFirma =
      this.rector[0].persona.nombre +
      ' ' +
      this.rector[0].persona.apellido +
      ' - ' +
      this.rector[0].uaaCargoNombre;
  ...
  registrar(archivo: File, firma: FirmaDigital) {
    ...
    this.firmaService.registrarFirma(arch, firma).subscribe(
      (data) => {
        this.mensajeSuccses();
  ...
  actualizar(element: FirmaDigital) {
    ...
    this.firmaService.actualizarFirma(firma).subscribe(
      (data) => {
        if (data > 0) {
          this.mensajeError();
          this.buscarFirmas();
  ...
  enviarEmailRector() {
    let nombreFirma =
      ...
      ' - ' +
      this.rector[0].uaaCargoNombre;
    let fecha: any = this.datePipe.transform(
      new Date(),
    ...
    this.firmaService
      .enviarEmailRector(
        this.rector[0].persona.emailInterno,
        this.foto.url,
        nombreFirma,
        fecha,
        nombre,
        this.persona[0].emailInterno,
        this.admin[0].cargoNombre
      )
      .subscribe(
        (data) => {
          if (data.estado == true) {
    ...
    cargar.percentageChanges().subscribe((porcentaje) => {
      referencia.getDownloadURL().subscribe((URL) => {
        ...
        this.foto.url = this.foto.url
          .replace(
            'https://firebasestorage.googleapis.com/v0/b/doctoradocienciasdelasaludusco.appspot.com/o/CARNETIZACION%2FFIRMADIGITAL%2F',
            ''
          )
          .replace('?alt=media&token=', 'url');
  ...
  mensajeError() {
    swal.fire({
      icon: 'error',
      title: 'Error.',
      text: 'El proceso no se pudo culminar.',
