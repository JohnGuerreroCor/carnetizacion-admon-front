...
export class TiqueteEventosComponent implements OnInit {
  ...
  limiteVigencia() {
    this.fechaLimiteMinimaVigencia = this.datePipe.transform(this.formularioTiquetesEvento.get('fechaInicio')!.value, 'yyyy-MM-dd');
  }
  ...
  agruparRegistros() {
    for (let index = 0; index < this.personas.length; index++) {
      this.personaService.obtenerPersonaPorIdentificacion(this.personas[index].Identificacion).subscribe(data => {
        if (JSON.stringify(data) !== '[]') {
          this.persona.push(data[0]);
          this.per = this.per + 1;
        } else {
          this.terceroService.getTerceroId(this.personas[index].Identificacion).subscribe(data => {
            if (JSON.stringify(data) !== '[]') {
              this.tercero.push(data[0]);
              this.ter = this.ter + 1;
              if ((this.ter + this.per + this.insert) == this.personas.length) {
                this.logo = false;
              }
            } else {
  ...
  verValores() {
    swal.fire({
      icon: 'success', title: 'Correos enviados', text: '¡Operación exitosa!',
      toast: true,
      position: 'top-right',
      timer: 2500
    })
  ...
  buscarPersonaEvento() {
    ...
    this.emails = [];
    this.personaService.obtenerPersonaPorIdentificacion(this.formularioTiquetesEvento.get('identificacion')!.value).subscribe(data => {
      if (JSON.stringify(data) !== '[]') {
        this.persona = data;
        ...
        this.formularioTiquetesEvento.get('codigo')!.setValue(this.persona[0].codigo);
        this.formularioTiquetesEvento.get('tipoDocumento')!.setValue(this.persona[0].tipoDocumento);
        this.formularioTiquetesEvento.get('nombre')!.setValue(this.persona[0].nombre);
        this.formularioTiquetesEvento.get('apellido')!.setValue(this.persona[0].apellido);
        this.formularioTiquetesEvento.get('email')!.setValue(this.persona[0].emailInterno);
      } else {
        swal.fire({
          icon: 'warning',
