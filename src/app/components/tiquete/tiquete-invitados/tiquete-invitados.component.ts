...
export class TiqueteInvitadosComponent implements OnInit {
  ...
  limiteVigencia() {
    this.fechaLimiteMinimaVigencia = new Date(
      this.formTercero.get('fechaInicio')!.value
    );
  }
      ...
      .subscribe((data) => {
        if (JSON.stringify(data) !== '[]') {
          ...
          this.carnetNombre =
            ...
            ' ' +
            nombres[1].toUpperCase() +
            ' ' +
            apellidos[0].toUpperCase() +
            ' ' +
            apellidos[1].toUpperCase();
          this.carnetCorreo = this.formTercero.get('email')!.value;
          this.carnetId = this.formTercero.get('identificacion')!.value;
          this.carnetInicio = this.datePipe.transform(
            this.formTercero.get('fechaInicio')!.value,
            'dd-MM-yyyy'
          );
          this.carnetVigencia = this.datePipe.transform(
            this.formTercero.get('fechaVigencia')!.value,
            'dd-MM-yyyy'
          );
  ...
  subirArchivo() {
    ...
    cargar.percentageChanges().subscribe((porcentaje) => {
      referencia.getDownloadURL().subscribe((URL) => {
        ...
        this.foto.url = this.foto.url
          .replace(
            'https://firebasestorage.googleapis.com/v0/b/doctoradocienciasdelasaludusco.appspot.com/o/CARNETIZACION%2FFOTOS%2F',
            ''
          )
